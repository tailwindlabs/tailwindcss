...
function Color({ name, value }) {
  return (
    <div class="flex items-center mt-5">
      <div
        class="h-12 w-12 rounded-lg shadow-inner"
        style={{
  ...
  return (
    <div className="flex flex-wrap -mx-2 mt-0">
      {colors.map((color, i) => {
        ...
        if (palette.length < 3) {
          return (
            <div class={`px-2 w-full relative ${i === 0 ? '' : 'mt-4'}`}>
              <h3 class="markdown no-toc mb-4 mt-8">{title}</h3>
        ...
        return (
          <div class={`w-1/2 px-2 md:w-full relative ${i === 0 ? '' : 'mt-4'}`}>
            <h3 class="markdown no-toc mb-4 mt-8">{title}</h3>
