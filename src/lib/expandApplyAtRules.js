...
function extractClasses(node) {
  ...
  const container = postcss.root({ nodes: [node.clone()] })

  container.walkRules((rule) => {
    parser((selectors) => {
      selectors.walkClasses((classSelector) => {
        const parentSelector = classSelector.parent.toString()
  ...
  const normalizedGroups = Array.from(groups.values(), (classes) => Array.from(classes))
  const classes = normalizedGroups.flat()

  return Object.assign(classes, { groups: normalizedGroups })
}

const selectorExtractor = parser()
...
function nestedClone(node) {
  for (const parent of pathToRoot(node)) {
    if (node === parent) {
      continue
    }
...
function buildLocalApplyCache(root, context) {
  ...
  root.walkRules((rule) => {
    ...
    for (const node of pathToRoot(rule)) {
    ...
    const container = nestedClone(rule)
    const sort = context.offsets.create('user')

    for (const className of extractClasses(rule)) {
      const list = cache.get(className) || []
...
function processApply(root, context, localCache) {
  ...
  // Fill up some caches!
  const applyClassCache = combineCaches([localCache, buildApplyCache(applyCandidates, context)])
  ...
  function replaceSelector(selector, utilitySelectors, candidate) {
    const selectorList = extractSelectors(selector)
    const utilitySelectorsList = extractSelectors(utilitySelectors)
    const candidateList = extractSelectors(`.${escapeClassName(candidate)}`)
    const candidateClass = candidateList.nodes[0].nodes[0]
  ...
  for (const apply of applies) {
    const [candidates] = perParentApplies.get(apply.parent) || [[], apply.source]
  ...
  for (const [parent, [candidates, atApplySource]] of perParentApplies) {
    ...
    for (const [applyCandidate, important, rules] of candidates) {
      const potentialApplyCandidates = [
        applyCandidate,
        ...extractBaseCandidates([applyCandidate], context.tailwindConfig.separator),
      ]

      for (const [meta, node] of rules) {
        const parentClasses = extractClasses(parent)
        ...
        const intersects = parentClasses.some((selector) => nodeClasses.includes(selector))
        if (intersects) {
          throw node.error(
        ...
        const root = postcss.root({ nodes: [node.clone()] })
        ...
        if (canRewriteSelector) {
          root.walkRules((rule) => {
            ...
            const isGenerated = parent.raws.tailwind !== undefined
    ...
    const nodes = context.offsets.sort(siblings).map((s) => s[1])
...
export default function expandApplyAtRules(context) {
  return (root) => {
    ...
    const localCache = lazyCache(() => buildLocalApplyCache(root, context))
