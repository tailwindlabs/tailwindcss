...
[Robin](https://twitter.com/malfaitrobin) came up with a [really cool solution](https://github.com/tailwindlabs/headlessui/pull/1432) for this that is super simple — instead of trying to manually control how tabbing works, just throw an invisible focusable element at the beginning of the focus trap and another one at the end. Now whenever the one at the end receives focus, you know to immediately focus the first item in the focus trap, and when the invisible element at the beginning receives focus, you know to focus the last item in the focus trap.

With this approach, you don't have to hijack the tab key at all — you just let the browser do all of the work and move focus whenever one of your sentinel elements receives focus.
...
This is available now for anyone with a Tailwind UI team account — just open the dropdown menu and click "My Team" to name your team and invite your co-workers!
