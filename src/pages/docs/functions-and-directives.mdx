...
`@apply` did not work with pseudo-class or responsive variants of another utility. Howerve since v2.x we made this behaviour work as expected.

```css
/* This now works as expted: */
.btn {
...
Use the `@layer` directive to tell Tailwind which "bucket" a set of custom styles belong in. Valid layers are a `base`, `components`, and `utilities`.
...
Wrapping any custom CSS in a `@layer` directive also tells Tailwind to consider those styles for purging when purging that layer. Read our [Optimizing for Production](/docs/optimizing-for-production) documentation for more details.
