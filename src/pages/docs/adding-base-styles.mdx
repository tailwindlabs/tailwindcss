...
Using the `@layer` directive will also instruct Tailwind to consider those styles for purging when purging the `base` layer. Read our [Optimizing for Production](/docs/optimizing-for-production) documentation for more details.
