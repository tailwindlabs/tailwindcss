...
Using the `@layer` directive will also instruct Tailwind to consider those styles for purging when purging the `utilities` layer. Read our [Optimizing for Production](/docs/optimizing-for-production) documentation for more details.
...
If you'd like to create [state variants](/docs/hover-focus-and-other-states) for your own utilities, wrap your utilities in the `@variants` directive:
...
If you'd like to generate both responsive _and_ state variants of your custom utilities, wrap `@variants` with `@responsive`:
