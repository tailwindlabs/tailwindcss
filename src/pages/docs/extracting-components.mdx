...
Using the `@layer` directive will also instruct Tailwind to consider those styles for purging when purging the `components` layer. Read our [Optimizing for Production](/docs/optimizing-for-production) documentation for more details.
...
@layer components {
  ...
  .btn-gray {
    @apply bg-gray-400 hover:bg-gray-500 text-gray-800 font-bold py-2 px-4 rounded;
  }
}
```
