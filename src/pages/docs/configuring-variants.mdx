...
The `variants` section of your `tailwind.config.js` file is where you control which core utility plugins should have [responsive variants](/docs/responsive-design) and [state variants](/docs/hover-focus-and-other-states) generated.
...
Because [the order of variants is important](/docs/configuring-variants#ordering-variants), we provided a `variantOrder` key in the config section that makes it easy to add new variants in the right place. You can also order them yourself inside the `variants` section directly.
