...
The `variants` section of your `tailwind.config.js` file is where you control which core plugins should have [responsive variants](/docs/responsive-design), [dark mode variants](/docs/dark-mode) and [state variants](/docs/hover-focus-and-other-states) generated.
...
For more information about how variants work, read our documentation on [hover, focus and other states](/docs/hover-focus-and-other-states).
...
Because [the order of variants is important](/docs/configuring-variants#ordering-variants), any variants added under the `extend` key are automatically ordered for you using a sensible default variant order. You can customize this order using the `variantOrder` key if necessary.
...
It's also important to note that we have a default `variantOrder` config in case you are extending variants. If you want to change the order for your extended variants you have 2 options:

The first option is to update the `variantOrder`.
...
The second option is to change the order by explictly setting all the variants for a certain plugin, instead of extending the variants for that plugin.
...
module.exports = {
  variants: {
    // ...
+   backgroundColor: ['responsive', 'active', 'group-hover', 'hover', 'focus'],
    extend: {
      // ...
-     backgroundColor: ['active']
    }
...
Note that enabling a lot of variants for all plugins will result in much bigger file sizes. Before you do this, be sure to read our guide on [Optimizing for Production](/docs/optimizing-for-production/).
