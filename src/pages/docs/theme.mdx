...
We provide a sensible [default theme](https://github.com/tailwindlabs/tailwindcss/blob/v1/stubs/defaultConfig.stub.js#L5) with a very generous set of values to get you started, but don't be afraid to change it or extend; you're encouraged to customize it as much as you need to fit the goals of your design.
...
See the [theme configuration reference](#configuration-reference) or the [default theme](https://github.com/tailwindlabs/tailwindcss/blob/v1/stubs/defaultConfig.stub.js#L5) for a complete list of theme options.
...
When you use the `extend` keyword then all the objects underneath will be deeply merged. Arrays are an exception to this rule. If you want to define a new font family with a set of fonts, you can safely use an array. If you do want to append to an existing font family, then you can spread the items from the default config.

There is an exception to this. If your array has objects in it, then we will merge the arrays together. This is useful for plugin authors if they accept arrays of objects where the keys of each object could collide.
...
### Special theme values

Sometimes values are implemented as arrays (for example in the case of `fontFamily`). Sometimes there are complex values like `fontSize` which looks like a tuple:
...
In this case, when you use the `theme('fontSize.large')` function you will receive the first value (`12rem`) instead of the tuple. If you do want to get the original value, then you have to use the `config()` function instead.

The following plugins will return the first value of the tuple, if the configuration value is defined as a tuple.

- `fontSize`
- `outline`

The following plugins will return the value as a comma separated string, if it is defined as an array.
