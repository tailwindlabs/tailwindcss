...
1. [Replace `@tailwind preflight` with `@tailwind base`](#4-replace-preflight-with-base)
1. [Replace `config()` with `theme()`](#5-replace-config-with-theme)
1. [Explicitly style any headings](#6-explicitly-style-any-headings)
1. [Explicitly style any lists that should have bullets/numbers](#7-explicitly-style-any-lists)
1. [Remove any usage of `.list-reset`](#8-remove-any-usage-of-list-reset)
1. [Replace `.pin-{side}` with `.{top|left|bottom|right|inset}-{value}`](#9-replace-pin-with-inset)
...
1. [Add `inline` to any replaced elements (`img`, `video`, etc.) that should not be `display: block`](#13-add-inline-to-any-replaced-elements-img-video-etc-that-should-not-be-display-block)
