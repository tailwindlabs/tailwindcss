...
describe('variant stacking', () => {
  it('should stack simple variants', async () => {
    expect(await run(['focus:hover:flex'])).toMatchInlineSnapshot(`
      "
      @media (hover: hover) {
        :is(.focus\\:hover\\:flex:focus):hover {
          display: flex;
        }
      }
      "
  ...
  it('pseudo element variants are re-ordered', async () => {
    expect(await run(['before:hover:flex', 'hover:before:flex'])).toMatchInlineSnapshot(`
      ...
      @media (hover: hover) {
        :is(.before\\:hover\\:flex::before):hover {
          display: flex;
        }
        :is(.hover\\:before\\:flex:hover)::before {
          content: var(--tw-content);
          display: flex;
        }
      }
...
describe('sorting', () => {
  ...
  it('should sort variants and stacked variants by variant position', async () => {
    expect(
      ...
      @media (hover: hover) {
        :is(.hover\\:focus\\:flex:hover):focus {
          display: flex;
        }
      }
  ...
  it('should order group-* and peer-* variants based on the sort order of the group and peer variant but also based on the variant they are wrapping', async () => {
    expect(
      ...
      @media (hover: hover) {
        ...
        @media (hover: hover) {
          :is(.group-hover\\:peer-hover\\:flex:is(:where(.group):hover *)):is(:where(.peer):hover ~ *) {
            display: flex;
          }
          :is(.peer-hover\\:group-hover\\:flex:is(:where(.peer):hover ~ *)):is(:where(.group):hover *) {
            display: flex;
          }
        }
        :is(.group-focus\\:peer-hover\\:flex:is(:where(.group):focus *)):is(:where(.peer):hover ~ *) {
          display: flex;
        }
        :is(.peer-hover\\:group-focus\\:flex:is(:where(.peer):hover ~ *)):is(:where(.group):focus *) {
          display: flex;
        }
      }
      ...
      @media (hover: hover) {
        :is(.group-hover\\:peer-focus\\:flex:is(:where(.group):hover *)):is(:where(.peer):focus ~ *) {
          display: flex;
        }
        :is(.peer-focus\\:group-hover\\:flex:is(:where(.peer):focus ~ *)):is(:where(.group):hover *) {
          display: flex;
        }
      }
      :is(.group-focus\\:peer-focus\\:flex:is(:where(.group):focus *)):is(:where(.peer):focus ~ *) {
        display: flex;
      }
      :is(.peer-focus\\:group-focus\\:flex:is(:where(.peer):focus ~ *)):is(:where(.group):focus *) {
        display: flex;
      }
      @media (hover: hover) {
        .hover\\:flex:hover {
...
describe('@custom-variant', () => {
    ...
    test('grouped nested selectors with @slot', async () => {
      expect(
        ...
        @layer utilities {
          ...
          :is(.custom-before\\:underline::before):hover, :is(.custom-before\\:underline::before):focus {
            text-decoration-line: underline;
          }
        }
        "
  ...
  test('@custom-variant can use a @variant that eventually uses another @custom-variant (2)', async () => {
    expect(
      ...
      :is(:is(.b\\:flex .b) .a) .a-inside-b {
        display: flex;
      }
      :is(:is(:is(.a\\:b\\:flex .a) .b) .a) .a-inside-b {
        display: flex;
      }
      :is(:is(:is(.b\\:a\\:flex .b) .a) .a-inside-b) .a {
        display: flex;
      }
      "
    `)
  ...
  test('@custom-variant can use existing @slot @variants', async () => {
    expect(
      ...
      :is(.hocus\\:flex[data-hover]):focus {
        display: flex;
      }
      "
    `)
  ...
  test('@custom-variant setup that results in a circular dependency error can be solved', async () => {
    expect(
      ...
      "
      [data-broken-circle] :is(.baz\\:flex:active) {
        display: flex;
      }
      "
    `)
...
describe('@variant', () => {
  ...
  it('should be possible to use nested `@variant` rules', async () => {
    expect(
      ...
      :is(.btn:disabled):focus {
        background: white;
      }
      :is(.disabled\\:focus\\:underline:disabled):focus {
        text-decoration-line: underline;
      }
      "
    `)
  ...
  describe('stacked `@variant` rules', () => {
    ...
    it('should handle variants containing a `:` inside', async () => {
      expect(
        ...
        :is(.btn:is(:hover, :focus)):disabled {
          background: red;
        }
        @media (hover: hover) {
          :is(.btn[aria-disabled="true"]):hover {
            background: red;
          }
        }
        "
  ...
  it('should be possible to use compound and stacked variants in `@variant`', async () => {
    ...
    expect(await compileCss(input)).toMatchInlineSnapshot(`
      ...
      :is(.btn[data-a])[data-d] {
        background: blue;
      }
      :is(:is(.btn[data-a])[data-e])[data-f] {
        background: blue;
      }
      :is(.btn[data-b])[data-c] {
        background: red;
      }
      :is(:is(.btn[data-b])[data-c])[data-d] {
        background: blue;
      }
      :is(:is(:is(.btn[data-b])[data-c])[data-e])[data-f] {
        background: blue;
      }
      "
    `)
