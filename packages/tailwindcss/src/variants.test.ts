...
test('not', async () => {
  expect(
    ...
    .not-\\[\\:checked\\]\\:flex:not(:is(:checked)) {
      display: flex;
    }
    @media not (orientation:landscape) {
      .not-\\[\\@media_\\(orientation\\:landscape\\)\\]\\:flex {
    ...
    .group-not-\\[\\:checked\\]\\:flex:is(:where(.group):not(:is(:checked)) *) {
      display: flex;
    }
    .group-not-\\[\\:checked\\]\\/parent-name\\:flex:is(:where(.group\\/parent-name):not(:is(:checked)) *) {
      display: flex;
    }
    ...
    .peer-not-\\[\\:checked\\]\\:flex:is(:where(.peer):not(:is(:checked)) ~ *) {
      display: flex;
    }
    .peer-not-\\[\\:checked\\]\\/sibling-name\\:flex:is(:where(.peer\\/sibling-name):not(:is(:checked)) ~ *) {
      display: flex;
    }
    "
  `)
...
test('in', async () => {
  expect(
    ...
    "
    .not-in-\\[\\.group\\]\\:flex:not(:where(:is(.group)) *) {
      display: flex;
    }
    .not-in-\\[p\\]\\:flex:not(:where(:is(p)) *) {
      display: flex;
    }
    :where([data-visible]) .in-data-visible\\:flex {
      display: flex;
    }
    :where(:is(.group)) .in-\\[\\.group\\]\\:flex {
      display: flex;
    }
    :where(:is(p)) .in-\\[p\\]\\:flex {
      display: flex;
    }
    "
  `)
...
test('has', async () => {
  expect(
    ...
    .group-has-\\[\\:checked\\]\\:flex:is(:where(.group):has(:is(:checked)) *) {
      display: flex;
    }
    .group-has-\\[\\:checked\\]\\/parent-name\\:flex:is(:where(.group\\/parent-name):has(:is(:checked)) *) {
      display: flex;
    }
    ...
    .peer-has-\\[\\:checked\\]\\:flex:is(:where(.peer):has(:is(:checked)) ~ *) {
      display: flex;
    }
    .peer-has-\\[\\:checked\\]\\/sibling-name\\:flex:is(:where(.peer\\/sibling-name):has(:is(:checked)) ~ *) {
      display: flex;
    }
    ...
    .has-\\[\\:checked\\]\\:flex:has(:is(:checked)) {
      display: flex;
    }
...
test('data', async () => {
  ...
  expect(await run(['data-[état]:flex', 'aria-[état]:flex'])).toMatchInlineSnapshot(`
    "
    .aria-\\[état\\]\\:flex[aria-état] {
      display: flex;
    }
    .data-\\[état\\]\\:flex[data-état] {
      display: flex;
    }
    "
  `)
...
test('matchVariant sorts deterministically', async () => {
  ...
  for (let classList of classLists) {
    expect(
      ...
      "
      .is-data\\:flex:is([data-default]) {
        display: flex;
      }
      .is-data-foo\\:flex:is([data-foo]) {
        display: flex;
      }
      .is-data-bar\\:flex:is([data-bar]) {
        display: flex;
      }
      .is-data-\\[potato\\]\\:flex:is([data-potato]) {
        display: flex;
      }
      .is-data-\\[sandwich\\]\\:flex:is([data-sandwich]) {
        display: flex;
      }
      "
    `)
